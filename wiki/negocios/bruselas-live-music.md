---
title: "Bruselas Live Music"
type: negocio
negocio: bruselas
tags: [bodas, eventos, jazz, musica-en-vivo, wedding-planners, meta-ads, b2b]
created: 2026-06-27
updated: 2026-06-27
estado: activo
---

# Bruselas Live Music (Páramo)
_Conglomerado de proyectos de música en vivo para bodas, corporativos y eventos privados. Jazz, bossa nova, música francesa, brass band. Instagram: @bruselaslivemusic._

**Dos nombres según el mercado**: **Páramo** — usado en cotizaciones y contratos; **Bruselas Live Music** — nombre comercial para el mercado de planners/venues.

## Qué es

No es una banda fija — es un conglomerado de formatos y proyectos de música en vivo que Ernesto dirige:
- Dueto voz/guitarra
- Ensamble de jazz

> **Nota (jun 2026):** "Bruselas" como dúo artístico (guitarra + voz, repertorio jazz/contemporáneo) tiene una dimensión artística propia separada de la banda de bodas. El nombre "Bruselas Live Music" agrupa ambas facetas comercialmente, pero el dúo artístico tiene objetivos de desarrollo propios. Ver [[wiki/negocios/nikan]] para el proyecto de composición original.
- Brass band
- Otros formatos según el evento

Géneros: jazz, bossa nova, música francesa, manouche/gypsy-jazz (bajo **Nuages**).

## Oferta y precios

| Formato | Ticket promedio |
|---|---|
| Boda | $12,000–$25,000 MXN |
| Corporativo / privado | (no especificado) |

**Meta actual**: 1 evento/mes.
**Presupuesto sostenible de marketing**: <$2,000 MXN/mes.

## Estado actual

### B2B (canal preferido de Ernesto)
- 21 wedding planners mapeados en Guadalajara/Zapopan/Tlaquepaque con teléfono, email y sitio en seguimiento
- Archivo: `wedding_planners_guadalajara_seguimiento.xlsx`
- Emails de outreach redactados (ES/EN): primer contacto sin precios, posicionándose como proveedor recomendado
- Regla de seguimiento: 1 segundo toque a los 5–7 días si no hay respuesta; después, pasar a la siguiente cuenta
- **Email Brevo a ~80 planners**: programado para 2026-06-30 (mañana al momento de escribir esto)
- **Campaña WhatsApp**: en curso esta semana a planners ya conocidos (warm outreach)
- Dominio propio `bruselaslivemusic.com` (Namecheap), sender `info@bruselaslivemusic.com` verificado

### bodas.mx — evaluado y descartado (jun 2026)
- **Oferta recibida**: $18,000 MXN/año (descuento desde $24,000)
- **Evaluación de Ernesto**: no es su nicho. La oferta de proveedores en la plataforma no coincide con el posicionamiento de Bruselas. Grupos aparentemente inactivos.
- **Decisión**: no tomado. La señal del mercado (grupos inactivos, perfil de proveedor desalineado) no justifica el gasto.

> ✓ Confirmado: bodas.mx descartado. El canal B2B directo (WhatsApp + Brevo a planners conocidos) es más eficiente y sin costo fijo.

### Pendientes activos
> ⚠️ Pendiente: Implementar registro simple de conversaciones (fecha, canal, cerró sí/no, valor). Hoy Ernesto responde DMs/WhatsApp sin registrar origen ni resultado — el CRM es un agujero negro.
> ⚠️ Pendiente: Ampliar lista de wedding planners más allá de los 21 mapeados (tarea mecánica → delegar a VA/Fiverr).
> ⚠️ Pendiente: Seguimiento a la campaña fría de Brevo (~80 planners).

## Métricas clave

### Meta Ads histórico
| Estrategia | Resultado |
|---|---|
| Objetivo "Mensajes" / venta directa | $40–127 MXN/conversación sin cierres medibles |
| Objetivo Interacción + CTA de palabra clave | A validar |

### Contenido orgánico
> ✓ Confirmado: Recopilatorios de "momentos reales" de bodas/eventos rinden por mucho mejor. Un solo post puede generar 80%+ de views/seguidores nuevos del mes.
> ✓ Confirmado: Contenido tipo catálogo ("elige tu formato", "jazz a la carta") rinde mal (cientos de views).

## Reglas de operación

### Meta Ads
- **Objetivo**: Interacción — NUNCA "Mensajes" (históricamente sin cierres medibles)
- **Presupuesto de prueba**: $300–500 MXN
- **CTA**: palabra clave ("Comenta COTIZACIÓN" o "Comenta BODA") en vez de venta directa
- Solo reforzar con pauta contenido que ya validó tracción orgánica

### Calendario (verificar SIEMPRE antes de lanzar pauta)
- **Evitar**: Mundial y eventos deportivos masivos, festividades sin bodas
- **Aprovechar**: Día de las Madres, aniversarios, temporada alta de bodas en Guadalajara (octubre–mayo)

### Delegación
- Investigación y listas de planners → VA/Fiverr (mecánico)
- Relación con cliente/planner → Ernesto personalmente (la credibilidad musical cierra la boda)

## Aprendizajes clave

> ✓ Confirmado: El canal B2B (wedding planners y venues) es la ruta de adquisición preferida y más eficiente para tickets altos.
> ✓ Confirmado: La falta de registro de conversaciones impide medir qué canal realmente cierra. Agujero negro recurrente.

## Conexiones
- [[wiki/contactos/wedding-planners-guadalajara]] — lista de 21 planners mapeados
- [[wiki/canales/b2b-outreach]] — estrategia y emails de outreach
- [[wiki/canales/meta-ads]] — reglas de pauta para Bruselas
- [[wiki/herramientas/brevo]] — campaña fría a planners
- [[wiki/contratos/contrato-paramo]] — plantilla de contrato para bodas
- [[wiki/herramientas/apps-propias]] — calculadora de cotizaciones (misma lógica que EMQ)
- [[wiki/negocios/nuages]] — jazz manouche, uno de los formatos de Bruselas
- [[wiki/areas/ventas/principio-conversion]] — principio rector
